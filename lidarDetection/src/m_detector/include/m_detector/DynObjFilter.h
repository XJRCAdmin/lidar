...
struct point_soph
{
    ...
    std::array<Vector3i, HASH_PRIM> last_positions = {};   
...
public:
    ...
    {   
        ...
        std::fill_n(min_depth_static, MAX_2D_N, 0.0f);
        std::fill_n(min_depth_all, MAX_2D_N, 0.0f);
...
public:
  std::deque<DepthMap::Ptr> depth_map_list;
    ...
    DynObjFilter(float windows_dur,
    ...
    // main filter entry (same signature otherwise)
    void filter(PointCloudXYZI::Ptr feats_undistort,
                const M3D &rot_end,
                const V3D &pos_end,
                const double & scan_end_time);

    // publishers use ROS2 publisher shared_ptrs (sensor_msgs::msg::PointCloud2)
    void publish_dyn(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pub_point_out,
    ...
    void set_path(const std::string &file_path, const std::string &file_path_origin);

    // internal algorithm methods (unchanged signatures)
