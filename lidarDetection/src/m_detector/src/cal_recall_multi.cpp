...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("check_dynamic");
    Init();

    int dataset = node->declare_parameter<int>("dyn_obj.dataset", -1);
    bool is_origin = node->declare_parameter<bool>("dyn_obj.is_origin", false);
    dataset_folder = node->declare_parameter<std::string>("dyn_obj.dataset_folder", "/");
    int start_param = node->declare_parameter<int>("dyn_obj.start_param", -1);
    int end_param = node->declare_parameter<int>("dyn_obj.end_param", 0);
    int start_se = node->declare_parameter<int>("dyn_obj.start_se", -1);
    int end_se = node->declare_parameter<int>("dyn_obj.end_se", 0);
